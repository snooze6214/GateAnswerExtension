...
window.onload = () => {
    const modal_title = document.querySelector('#exampleModalLongTitle').innerText.replaceAll('\t', "");
    const subject = modal_title.split('\n')[2];
    ...
    try {
        fetch(`https://raw.githack.com/snooze6214/GateAnswerExtension/main/answers_${subject}.json`)
            .then(res => {
                if (res.ok)
                    return res.json();
        ...
        let error_display = document.createElement('p');
        error_display.textContent = 'GATE Marks extension error: Answer key not available for this subject';
        error_display.style.color = 'red';
        above_table_text[0].insertAdjacentElement("beforebegin", error_display);
    }
};
